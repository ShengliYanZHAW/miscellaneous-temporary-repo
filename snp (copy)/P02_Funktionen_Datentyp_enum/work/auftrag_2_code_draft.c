...
int main(int argc, const char *argv[])
{
    ...
    printf("Enter a valid date in the format yyyy-mm-dd: ");
    if (scanf("%10s", userInput) != 1) {
        fprintf(stderr, "Error: Input is missing or invalid format.\n");
        return EXIT_FAILURE;
    }

    int inputControl = sscanf(userInput, "%i-%i-%i", &newDate.year, &newDate.month, &newDate.day);

    // Checks if date and input is valid
    if (!is_valid_date(newDate) || (inputControl != 3)) {
        fprintf(stderr, "Error: Input is not a valid date.\n");
        return EXIT_FAILURE;
    }
